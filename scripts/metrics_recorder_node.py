...
class MetricsRecorderNode:
    def __init__(self, args) -> None:
        ...
        self.robot_state_sub = rospy.Subscriber(self.topics_names["pose"], PoseWithCovarianceStamped,
                                                callback=self.robot_state_callback)
        self.value_function_sub = rospy.Subscriber(self.topics_names["value_function_at_state"], PoseStamped,
                                                   callback=self.value_function_callback)
        self.failure_sub = rospy.Subscriber(self.topics_names["failure_set_at_state"], PoseStamped,
                                            callback=self.failure_callback)
        self.vlm_inference_time_sub = rospy.Subscriber(self.topics_names["vlm_inference_time"], Float32,
                                                       callback=self.vlm_inference_time_callback)

        self.constraint_map_sub = rospy.Subscriber(self.topics_names['constraints_grid_map'], OccupancyGrid,
                                                   callback=self.constraints_map_callback)
        self.nominal_planning_time_sub = rospy.Subscriber(self.topics_names["nominal_planning_time"], Float32,
                                                          self.nominal_planning_time_callback)
        self.safe_planning_time_sub = rospy.Subscriber(self.topics_names["safe_planning_time"], Float32,
                                                       callback=self.safe_planning_time_callback)
        self.brt_computation_time_sub = rospy.Subscriber(self.topics_names["brt_computation_time"], Float32,
                                                         callback=self.brt_computation_time_callback)
        self.grid_map_sub = rospy.Subscriber(self.topics_names["grid_map"], OccupancyGrid, self.grid_map_callback)
        self.semantic_map_sub = rospy.Subscriber(self.topics_names["semantic_grid_map"], OccupancyGrid,
                                                 self.semantic_map_callback)
        self.language_queries_sub = rospy.Subscriber(self.topics_names["language_constraint"], String,
                                                     callback=self.text_queries_callback)
    ...
    def save_all_metrics(self):
        ...
        np.save(os.path.join(self.save_path, now, "semantic_map_times.npy"),
                np.array(self.semantic_map_times, dtype=dtype))
        np.save(os.path.join(self.save_path, now, "combined_map_times.npy"),
                np.array(self.combined_map_times, dtype=dtype))
